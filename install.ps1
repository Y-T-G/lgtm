...
def main():
    try:
        ...
        prompt = (
            f"You are a second set of eyes reviewing shell commands before they execute. "
            f"The commands are generated by an experienced, trusted AI agent actively working on the user's machine. "
            f"The agent is highly capable, NOT untrustworthy. You are only here to catch rare, catastrophic oversights.\n\n"
            f"GUIDELINES:\n"
            f"1. DEFAULT TO ALLOWING the command. Trust the agent's intent. Normal development workflows (creating, modifying, or executing scripts, even in /tmp) are perfectly safe.\n"
            f"2. ALLOW all standard tools and complex scripts unconditionally.\n"
            f"3. DENY ONLY if the command is a clear and catastrophic mistake (e.g., 'rm -rf /', wiping critical system directories) or an explicit secret leak.\n\n"
